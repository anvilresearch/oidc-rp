...
var RelyingParty = function (_JSONDocument) {
  ...
  _createClass(RelyingParty, [{
    ...
    value: function register(options) {
      ...
      try {
        var _configuration = this.provider.configuration;

        assert(_configuration, 'OpenID Configuration is not initialized.');
        assert(_configuration.registration_endpoint, 'OpenID Configuration is missing registration_endpoint.');

        var uri = _configuration.registration_endpoint;
        var method = 'post';
    ...
    value: function jwks() {
      ...
      try {
        var _configuration2 = this.provider.configuration;

        assert(_configuration2, 'OpenID Configuration is not initialized.');
        assert(_configuration2.jwks_uri, 'OpenID Configuration is missing jwks_uri.');

        var uri = _configuration2.jwks_uri;
    ...
    value: function userinfo() {
      try {
        var _configuration3 = this.provider.configuration;

        assert(_configuration3, 'OpenID Configuration is not initialized.');
        assert(_configuration3.registration_endpoint, 'OpenID Configuration is missing registration_endpoint.');

        var uri = _configuration3.userinfo_endpoint;
    ...
    value: function logout() {
      try {
        var _configuration4 = this.provider.configuration;
        assert(_configuration4, 'OpenID Configuration is not initialized.');
        assert(_configuration4.end_session_endpoint, 'OpenID Configuration is missing end_session_endpoint.');
      } catch (error) {
        return Promise.reject(error);
      }
